...
Candidates are then restricted to source frames whose estimated pitch matches the target note exactly, and among the ten closest of those we choose one at random so as to add a bit more variation in the result.
...
A selected frame is frequently shorter than the target note it has to fill, in which case we use only what the source note contains rather than reading past its end into whatever follows it in the source recording.
Each placed segment is given a 5 ms fade at both ends, without which the joins between segments click audibly.

\section{Results}

For each reconstruction the pipeline reports the proportion of the target's duration covered by placed frames, the mean absolute difference in semitones between each target note and the source frame chosen for it, and the number of frames truncated because the chosen source note was shorter than the target note.
...
Unpitched sounds, most dog barks among them, yield no frames whatsoever, so a substantial fraction of each downloaded collection contributes nothing to the reconstruction; the analysis step now reports which sounds were dropped on this basis.
...
Another major improvement could come by using octave reduction. Some source frames match pitches in the target melodies but in a different octave. Labeling each source frame with its pitch class rather than its MIDI note number would increase the coverage of pitch classes that can be matched exactly.
